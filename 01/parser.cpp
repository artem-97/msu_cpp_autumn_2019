...
AST_node* Parser::term() {
  ...
  while (current_token.type == Type::MUL || 
         current_token.type == Type::DIV) {
    Token curr = current_token;
...
AST_node* Parser::expr() {
  ...
  while (current_token.type == Type::PLUS ||
    ...
    if (current_token.type == Type::PLUS) eat(Type::PLUS);
    else if (current_token.type == Type::MINUS) eat(Type::MINUS);
